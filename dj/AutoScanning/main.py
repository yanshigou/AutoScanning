...
def basic_info():
    white_list = ['渝D71P00', '渝AA6W72', '渝A015EY', '渝AZC452']
    try:
    ...
    while True:
        ...
        try:
            ...
            if status == "success":
                ...
                text.insert(tk.END, "\n%s【电警】【扫描到文件夹】 %s，【未发现图片】\n" % (now_time, qz_path))
                f.write("\n%s 【电警】【扫描到文件夹】 %s，【未发现图片】" % (now_time, qz_path))
            elif status == "error":
                text.insert(tk.END, "\n%s【电警】【上传出错，请查看日志】：%s\n" % (now_time, e))
    ...
    while True:
        ...
        try:
            ...
            if status == "success":
                ...
                text.insert(tk.END, '\n%s 【电警】再次推送未成功入库的数据: 【%s】' % (datetime.now(), car_id))
                f.write('\n%s 【电警】再次推送未成功入库的数据: 【%s】' % (datetime.now(), car_id))
            elif status == "fail":
                push_result = r.json().get('push_result')
                text.insert(tk.END, '\n%s 【电警】再次推送未成功入库的数据: 【%s】' % (datetime.now(), push_result))
                f.write('\n%s 【电警】再次推送未成功入库的数据: 【%s】' % (datetime.now(), push_result))
            else:
                car_id = r.json().get('car_id')
                push_result = r.json().get('push_result')
                text.insert(tk.END, '\n%s 【电警】再次推送未成功入库的数据: 【%s-%s】' % (datetime.now(), car_id, push_result))
                f.write('\n%s 【电警】再次推送未成功入库的数据: 【%s-%s】' % (datetime.now(), car_id, push_result))
            sleep(int(push_time))
        except Exception as e:
            print(e)
            f.write("%s 【电警】推送数据出错 %s\n" % (datetime.now(), str(e)))
            text.insert(tk.END, "%s 【电警】推送数据出错 %s\n" % (datetime.now(), str(e)))
        finally:
            f.close()
...
if __name__ == '__main__':
    ...
    if basic_info():
        ...
        root.title('电警扫描器v3.0.1_20200311')
        ...
        try:
            ...
            text.insert(tk.END, "%s 开始删除服务器上90天以前的【电警】取证图片\n" % datetime.now())
            f.write("%s 开始删除服务器上90天以前的【电警】取证图片\n" % datetime.now())
            res = requests.post('http://%s/djDataInfo/delOutTimeDJImage/' % ip_val).json()
            if res["status"] == "success":
                f.write("%s 删除服务器上90天以前的【电警】取证图片成功\n" % datetime.now())
                text.insert(tk.END, "删除服务器上90天以前的【电警】取证图片成功\n")
            else:
                f.write("%s 删除服务器上90天以前的【电警】取证图片失败 %s\n" % (datetime.now(), res['e']))
                text.insert(tk.END, "%s 删除服务器上90天以前的【电警】取证图片失败 %s\n" % (datetime.now(), res['e']))
        except Exception as e:
            print(e)
            f.write("%s 删除服务器上打包的【电警】文件出错，请检查服务器是否开启\n" % datetime.now())
            text.insert(tk.END, "%s 删除服务器上打包的【电警】文件出错，请检查服务器是否开启\n" % datetime.now())
        finally:
            f.close()
